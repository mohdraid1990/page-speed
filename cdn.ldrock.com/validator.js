(function (w, d) {
  ...
  function LV(config) {
    ...
    this.init = function (data) {
      ...
      try {
        this.browser_language = this.browser_language
          ? this.browser_language.split("-")[0].toUpperCase()
          : "EN";
      } catch (e) {
    ...
    this.tooltip = function (input, msg_code) {
      ...
      elem.style.top =
        input.getBoundingClientRect().top + w.pageYOffset - 10 + "px";
      elem.style.left =
        input.getBoundingClientRect().left + w.pageXOffset + "px";
      elem.style.maxWidth = input.getBoundingClientRect().width + "px";
      elem.style.top =
        parseInt(elem.style.top) - elem.getBoundingClientRect().height + "px";
    ...
    this.show_loading = function () {
      var id = "leadrock-" + Math.random().toString(36).substr(2, 17);
      var html = self.config.ww_html.replace(
        new RegExp(self.config.loader_id_replace, "g"),
        id
      );
